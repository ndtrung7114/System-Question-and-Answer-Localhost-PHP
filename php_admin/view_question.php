...
try {
   ...
   $sql_view = "SELECT question.question_id, question.user_id, user.name, module.module_name, question.title, question.content, question.image, question.date, question.time
   FROM question
   ...
   $comment_view = "SELECT comment.comment_id, user.name, comment.user_id, comment.comment_content, comment.date, comment.time, comment.image
   FROM comment
   INNER JOIN user 
   ...
   include '../pages_admin/view_question.html.php';
   $output = ob_get_clean();
